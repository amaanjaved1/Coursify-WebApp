...
export default function AddCoursesPage() {
  ...
  const queuedOrUploading = files.some(f => f.status === "queued" || f.status === "uploading")
  ...
  const toggleShowSkipped = (id: string) => {
    setFiles(prev => prev.map(f => f.id === id ? { ...f, showSkipped: !f.showSkipped } : f))
  }

  const toggleShowDuplicates = (id: string) => {
    setFiles(prev => prev.map(f => f.id === id ? { ...f, showDuplicates: !f.showDuplicates } : f))
  }
  ...
  const handleUploadAll = async () => {
    ...
    if (!session?.access_token) {
      setFiles(prev => prev.map(f =>
        f.status === "queued" ? { ...f, status: "error", error: "Not authenticated. Please sign in again." } : f
      ))
            ...
            <div className="p-6 space-y-4">
              ...
              {!isProcessing && (
                ...
                >
                  ...
                  <h3 className="text-xl font-semibold text-brand-navy dark:text-white mb-2">
                    {files.length === 0 ? "Drop your files here" : "Drop more files"}
                  </h3>
                  <p className="text-sm text-brand-navy/55 dark:text-white/50 text-center mb-6">
                    ...
                    <br />or click to browse (up to 5 files)
                  </p>
                  ...
                  {fileCountError && (
                    <div className="mt-5 flex items-start gap-3 p-4 rounded-2xl bg-red-500/10 dark:bg-red-400/10 border border-red-500/20 dark:border-red-400/15 w-full">
                      ...
                      <p className="text-sm text-red-700 dark:text-red-300">{fileCountError}</p>
                    </div>
                  )}
                </div>
              )}
                  ...
                  {files.map((entry, idx) => (
                    <div key={entry.id} className="rounded-2xl border border-brand-navy/10 dark:border-white/10 overflow-hidden">
                      ...
                      <div className="flex items-center gap-3 px-4 py-3 bg-brand-navy/[0.03] dark:bg-white/[0.03]">
                        ...
                        <span className="shrink-0 text-xs text-brand-navy/45 dark:text-white/40">
                            ...
                            <div className="rounded-xl bg-amber-500/10 dark:bg-amber-400/10 border border-amber-500/20 dark:border-amber-400/15 overflow-hidden">
                              ...
                              >
                                <span className="text-xs font-medium text-amber-800 dark:text-amber-300">
                                  {entry.result.duplicates.length} course{entry.result.duplicates.length !== 1 ? "s" : ""} already in DB
                                </span>
                            ...
                            <div className="rounded-xl bg-brand-navy/5 dark:bg-white/[0.06] border border-brand-navy/10 dark:border-white/10 overflow-hidden">
                              ...
                              >
                                <span className="text-xs font-medium text-brand-navy/80 dark:text-white/70">
                                  {entry.result.skipped.length} course{entry.result.skipped.length !== 1 ? "s" : ""} not in our DB yet
                                </span>
                                {entry.showSkipped
                      ...
                      {entry.status === "error" && entry.error && (
                        <div className="px-4 py-3 border-t border-brand-navy/8 dark:border-white/8">
                          <p className="text-xs text-red-700 dark:text-red-300">{entry.error}</p>
                        </div>
                      )}
                    </div>
                  ))}
              ...
              {isProcessing && (
                <div className="flex items-center justify-center gap-2 py-2">
                  ...
                  <span className="text-sm text-brand-navy/70 dark:text-white/60">Processing...</span>
                </div>
              )}
